...
export default function HotelBooking() {
  ...
  const { loading: loadingRooms, error: roomsError, data: roomsData } = useQuery(ROOMS, {
    variables: {
      where: {
        isAvailable: {
          equals: true,
  ...
  useEffect(() => {
    if (roomsData && roomsData.rooms) {
      const roomData = roomsData.rooms.map((room: any) => ({
        ...
        discount: calculateDiscount(room.roomType.actualPrice, room.roomType.offerPrice),
      ...
      const room = roomData.find((r: Room) => r.roomType === roomTypeFromQuery) || roomData[0];
  ...
  const calculateDiscount = (originalPrice: number, offerPrice: number) => {
    if (originalPrice > offerPrice) {
      const discountPercent = ((originalPrice - offerPrice) / originalPrice) * 100;
  ...
  const calculateTotalPrice = () => {
    ...
    if (mealOption === "with-breakfast") total += selectedRoom.breakfastPrice*(extraRooms + 1);
    if (mealOption === "with-breakfast-dinner") total += (selectedRoom.breakfastPrice + selectedRoom.dinnerPrice)*(extraRooms + 1);;
    return total;
  };
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        <div className="lg:col-span-2">
          {selectedRoom && (
            <Card className="shadow-lg p-6 mb-8 rounded-lg border border-gray-200 hover:shadow-xl transition-shadow duration-300">
              <CardHeader>
                <CardTitle className="text-3xl font-bold text-gray-800">{selectedRoom.roomType}</CardTitle>
                <CardDescription className="text-gray-600">{selectedRoom.description}</CardDescription>
              </CardHeader>
              <CardContent>
                ...
                <div className="flex justify-between items-center mb-6">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">
                      <span className="line-through">₹{selectedRoom.originalPrice.toLocaleString()}</span>
                      <span className="ml-2 text-green-600">{selectedRoom.discount}</span>
                    </p>
                  </div>
                  <div className="text-right">
                    ...
                    <p className="text-xs text-gray-500">({selectedRoom.reviewCount} reviews)</p>
                  </div>
                </div>
                <RadioGroup defaultValue="room-only" onValueChange={setMealOption}>
                  <div className="flex items-center space-x-2 mb-2">
                    <RadioGroupItem value="room-only" id="room-only" />
                    <Label htmlFor="room-only">Room Only</Label>
                  </div>
                  <div className="flex items-center space-x-2 mb-2">
                    <RadioGroupItem value="with-breakfast" id="with-breakfast" />
                    <Label htmlFor="with-breakfast">With Breakfast (+₹{selectedRoom.breakfastPrice})</Label>
                  </div>
                  <div className="flex items-center space-x-2">
                    <RadioGroupItem value="with-breakfast-dinner" id="with-breakfast-dinner" />
                    <Label htmlFor="with-breakfast-dinner">
                      With Breakfast & Dinner (+₹{selectedRoom.breakfastPrice + selectedRoom.dinnerPrice})
                    </Label>
                  </div>
                </RadioGroup>
              </CardContent>
              <CardFooter>
                <Button
                  className="w-full bg-[#C19A6B] text-white py-4 px-6 rounded-lg hover:bg-[#a8835b] text-base font-medium transition-all duration-300 shadow-md"
        ...
        <div>
          <Card className="p-6 shadow-lg rounded-lg border border-gray-200">
            <CardHeader>
              <CardTitle className="text-xl font-semibold text-gray-800">Occupancy Guidelines</CardTitle>
            </CardHeader>
            <CardContent>
              <ul className="list-disc list-inside space-y-3 text-gray-600">
                <li>Each room accommodates up to 2 adults and 1 child (age under 12 years).</li>
                <li>If an additional room is required, extra charges will apply.</li>
